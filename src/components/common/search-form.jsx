...
export function SearchForm({ onSearch, initialValues }) {
  ...
  const handleSearch = () => {
    if (onSearch) {
      ...
      // If no locations selected, show error or use empty search
      if (fromLocs.length === 0 || toLocs.length === 0) {
        const criteria = {
          ...
          searchCombinations: [], // Empty combinations
        };
        onSearch(criteria);
        return;
      }

      // Create all combinations of from-to pairs
      ...
      console.log("Search criteria with combinations:", criteria);
      onSearch(criteria);
    }
  };
