...
const FlightRouteMap = ({
  ...
  flightInfo,
  legs: propLegs,
  coordsMap: propCoordsMap,
  processedSearchData: propProcessedSearchData,
  className = "",
  height = "400px",
  showFlightPath = true,
  ...
  const processedSearchData = useMemo(() => {
    console.log("FlightRouteMap received data:", {
      searchData,
      flightInfo,
      propProcessedSearchData,
    });
    if (propProcessedSearchData) {
      console.log("Using prop processedSearchData:", propProcessedSearchData);
      return propProcessedSearchData;
    }
    const data =
      searchData ||
      (flightInfo && {
      ...
      null;
    console.log("Final processed search data:", data);
    return data;
  }, [searchData, flightInfo, propProcessedSearchData]);
  ...
  useEffect(() => {
    const parseLegs = async () => {
      ...
      // Debug log all props first
      console.log("FlightRouteMap Props:", {
        propLegs,
        propCoordsMap,
        propProcessedSearchData,
        hasPropLegs: !!propLegs,
        hasPropCoordsMap: !!propCoordsMap,
        hasPropProcessedSearchData: !!propProcessedSearchData,
      });

      // If we have props data, use it but resolve coordinates
      if (propLegs && propCoordsMap && propProcessedSearchData) {
        ...
        // Check if propLegs has the expected structure or needs transformation
        let processedLegs = propLegs;

        // If propLegs is an array of raw flight objects (like multi-city legs), transform them
        if (
          Array.isArray(propLegs) &&
          propLegs.length > 0 &&
          propLegs[0].flightNumber &&
          !propLegs[0].flight
        ) {
          console.log("🔄 Transforming raw flight legs to expected structure");
          processedLegs = propLegs.map((rawLeg, index) => {
            ...
            return {
              ...
              direction:
                propProcessedSearchData?.tripType === "multi_city"
                  ? `Chặng ${index + 1}`
        ...
        // Resolve coordinates for all airports in propCoordsMap
        const resolveCoordsPromises = Object.entries(propCoordsMap).map(
          async ([code, airport]) => {
        ...
        try {
          ...
          setCoordsMap(resolvedCoordsMap);
          setLoading(false);
        } catch (error) {
          console.error("❌ Error in Promise.allSettled:", error);
          // Fallback to original data
          setLegs(processedLegs);
          setCoordsMap(propCoordsMap);
          setLoading(false);
        }
      ...
      try {
        ...
        if (tripType === "one_way") {
          ...
          console.log("Processing round-trip data:", processedSearchData);
            ...
            parsedLegs = [
              {
                ...
                dep: processedSearchData.outboundFlight?.departureAirport || {
                  code: processedSearchData.from,
                  airportName:
                    processedSearchData.outboundFlight?.departure
                      ?.airportName ||
                    processedSearchData.outboundFlight?.departureAirport
                      ?.airportName ||
                    `Airport ${processedSearchData.from}`,
                  cityNames: [
                    processedSearchData.outboundFlight?.departure?.city ||
                      processedSearchData.outboundFlight?.departureAirport
                        ?.city ||
                      processedSearchData.from,
                  ],
                },
                arr: processedSearchData.outboundFlight?.arrivalAirport || {
                  code: processedSearchData.to,
                  airportName:
                    processedSearchData.outboundFlight?.arrival?.airportName ||
                    processedSearchData.outboundFlight?.arrivalAirport
                      ?.airportName ||
                    `Airport ${processedSearchData.to}`,
                  cityNames: [
                    processedSearchData.outboundFlight?.arrival?.city ||
                      processedSearchData.outboundFlight?.arrivalAirport
                        ?.city ||
                      processedSearchData.to,
                  ],
                },
                stops: Array.isArray(
              ...
              },
              {
                ...
                dep: processedSearchData.returnFlight?.departureAirport || {
                  code:
                    processedSearchData.returnFrom || processedSearchData.to,
                  airportName:
                    processedSearchData.returnFlight?.departure?.airportName ||
                    processedSearchData.returnFlight?.departureAirport
                      ?.airportName ||
                    `Airport ${
                      processedSearchData.returnFrom || processedSearchData.to
                    }`,
                  cityNames: [
                    processedSearchData.returnFlight?.departure?.city ||
                      processedSearchData.returnFlight?.departureAirport
                        ?.city ||
                      processedSearchData.returnFrom ||
                      processedSearchData.to,
                  ],
                },
                arr: processedSearchData.returnFlight?.arrivalAirport || {
                  code:
                    processedSearchData.returnTo || processedSearchData.from,
                  airportName:
                    processedSearchData.returnFlight?.arrival?.airportName ||
                    processedSearchData.returnFlight?.arrivalAirport
                      ?.airportName ||
                    `Airport ${
                      processedSearchData.returnTo || processedSearchData.from
                    }`,
                  cityNames: [
                    processedSearchData.returnFlight?.arrival?.city ||
                      processedSearchData.returnFlight?.arrivalAirport?.city ||
                      processedSearchData.returnTo ||
                      processedSearchData.from,
                  ],
                },
                stops: Array.isArray(
          ...
          // Check if we have direct propLegs for multi-city
          if (propLegs && Array.isArray(propLegs) && propLegs.length > 0) {
            console.log("Using propLegs for multi-city:", propLegs);
            parsedLegs = propLegs.map((leg, index) => ({
              flight: leg,
              dep: leg.departureAirport ||
                propCoordsMap[leg.departureAirportCode || leg.from] || {
              ...
              arr: leg.arrivalAirport ||
                propCoordsMap[leg.arrivalAirportCode || leg.to] || {
  ...
  useEffect(() => {
    console.log("Flight path rendering useEffect triggered:", {
      ...
      loading,
      processedSearchData: processedSearchData?.tripType,
    });
    ...
    console.log("Starting to render flight paths for", legs.length, "legs");
    ...
    legs.forEach((leg, legIndex) => {
      ...
      if (tripType === "round_trip" || tripType === "multi_city") {
        ...
        if (
          midPoint &&
          coordsMap[midPoint.from.code]?.lat &&
          coordsMap[midPoint.from.code]?.lon
        ) {
      ...
      segmentsArray.forEach((seg, segIndex) => {
        console.log(`Processing segment ${segIndex} for leg ${legIndex}:`, {
          from: seg.from?.code,
          to: seg.to?.code,
          hasFromCoords: !!coordsMap[seg.from?.code],
          hasToCoords: !!coordsMap[seg.to?.code],
          fromCoords: coordsMap[seg.from?.code],
          toCoords: coordsMap[seg.to?.code],
        });

        const fromCoords = coordsMap[seg.from.code];
        const toCoords = coordsMap[seg.to.code];
  ...
  const renderMarkers = useMemo(() => {
    ...
    if (legs.length === 0 || Object.keys(coordsMap).length === 0) return null;
