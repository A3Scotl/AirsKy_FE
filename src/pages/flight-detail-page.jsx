...
import { Clock, Plane, MapPin, Calendar, DollarSign, Luggage, Star, AlertCircle, ArrowRight, Check, Wifi, Monitor, Utensils, Zap, Package, Headphones, Bed, Users } from "lucide-react";
...
const FlightDetail = () => {
  ...
  const defaultFlightInfo = {
    ...
    flightNumber: "VN7210",
    airlineLogo: "https://logos-world.net/wp-content/uploads/2021/02/Vietnam-Airlines-Logo.png",
    departure: {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="text-center max-w-4xl mx-auto px-4">
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <h1 className="text-4xl font-bold">
                {flightInfo.departure?.city || flightInfo.from} → {flightInfo.arrival?.city || flightInfo.to}
              </h1>
            </div>
            <p className="text-xl mb-2">{flightInfo.airline} Flight {flightInfo.flightNumber || 'VN7210'}</p>
            <div className="flex items-center justify-center gap-8 text-lg">
              <span>{flightInfo.departure?.time || flightInfo.departureTime} - {flightInfo.arrival?.time || '07:10'}</span>
              <span>•</span>
      ...
      <div className="max-w-7xl mx-auto py-8 px-4">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="flex flex-col items-center text-center p-4 border rounded-lg">
                ...
                <p className="font-bold">{flightInfo.departure?.city || flightInfo.from} ({flightInfo.departure?.code || flightInfo.fromCode})</p>
                <p className="text-sm text-gray-600">
                  {flightInfo.departure?.time || flightInfo.departureTime}, {flightInfo.departure?.date || flightInfo.date}
                </p>
              </div>
              <div className="flex flex-col items-center text-center p-4 border rounded-lg">
                <Plane className="w-8 h-8 mb-3 text-blue-600" />
                <h3 className="font-semibold text-lg">Flight Details</h3>
                <p className="font-bold">{flightInfo.duration || defaultFlightInfo.duration} • {flightInfo.stops || defaultFlightInfo.stops}</p>
                <p className="text-sm text-gray-600">Aircraft: {flightInfo.aircraft || defaultFlightInfo.aircraft}</p>
              </div>
              <div className="flex flex-col items-center text-center p-4 border rounded-lg">
                <MapPin className="w-8 h-8 mb-3 text-blue-600" />
                <h3 className="font-semibold text-lg">Arrival</h3>
                <p className="font-bold">{flightInfo.arrival?.city || flightInfo.to} ({flightInfo.arrival?.code || flightInfo.toCode})</p>
                <p className="text-sm text-gray-600">
                  {flightInfo.arrival?.time || '07:10'}, {flightInfo.arrival?.date || flightInfo.date}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Choose Your Fare</h2>
          <p className="text-gray-600 mb-6">Select the cabin class that best suits your travel needs</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {fareClasses.map((fare) => (
              <div 
                ...
                } ${fare.popular ? 'border-blue-200 bg-blue-50 relative' : 'border-gray-200'}`}
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="text-center mb-4">
                    <h3 className="text-xl font-bold text-gray-900 mb-1">{fare.type}</h3>
                    <Badge variant="secondary" className="text-xs">{fare.availability}</Badge>
                  </div>

                  {/* Price */}
                  <div className="text-center mb-6">
                    <div className="flex items-center justify-center gap-2 mb-1">
                      <span className="text-3xl font-bold text-blue-600">${fare.price}</span>
                      <span className="text-lg text-gray-500 line-through">${fare.originalPrice}</span>
                    </div>
                  ...
                  <div className="space-y-3 mb-6">
                    {fare.features.slice(0, 4).map((feature, idx) => (
                      <div key={idx} className="flex items-center text-sm">
                        <span className={`mr-3 ${feature.included ? 'text-green-500' : 'text-red-500'}`}>
                          {feature.included ? <Check className="w-4 h-4" /> : '✗'}
                        </span>
                  ...
                  {selectedFare === fare.id ? (
                    <div className="space-y-3">
                      ...
                      >
                        Continue to Booking <ArrowRight className="w-4 h-4 ml-2" />
                      </Button>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-green-800">
                    {fareClasses.find(f => f.id === selectedFare)?.type} selected
                  </p>
                  <p className="text-xs text-green-600">
                    Total: ${fareClasses.find(f => f.id === selectedFare)?.price} per person
                  </p>
                </div>
                <Button 
                  size="sm"
            ...
            <div className="p-6">
              <TabsContent value="details" className="mt-0">
                <div className="space-y-6">
                  {/* Flight Timeline */}
                  <div>
                    <h3 className="text-lg font-semibold mb-4 text-gray-900">Flight Schedule</h3>
                    <div className="relative">
                      ...
                      <div className="flex items-start space-x-4 mb-8">
                        ...
                        <div className="flex-grow">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="text-lg font-semibold text-gray-900">Departure</h4>
                            <Badge variant="outline" className="text-green-600 border-green-600">On Time</Badge>
                          </div>
                          <p className="text-2xl font-bold text-gray-900">{flightInfo.departure.time}</p>
                          <p className="text-gray-600">{flightInfo.departure.date}</p>
                          <p className="text-lg font-medium text-gray-800 mt-1">{flightInfo.departure.airport}</p>
                          <p className="text-sm text-gray-500">Terminal 4, Gate A12</p>
                        </div>
                      </div>
                        ...
                        <div className="flex-grow border-l-2 border-dashed border-gray-300 pl-4">
                          <div className="bg-gray-50 rounded-lg p-3">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="font-semibold text-gray-900">{flightInfo.duration}</span>
                            </div>
                            <div className="flex items-center justify-between mt-1">
                              <span className="text-sm text-gray-600">Aircraft</span>
                              <span className="font-semibold text-gray-900">{flightInfo.aircraft}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-grow">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="text-lg font-semibold text-gray-900">Arrival</h4>
                            <Badge variant="outline" className="text-green-600 border-green-600">On Time</Badge>
                          </div>
                          <p className="text-2xl font-bold text-gray-900">{flightInfo.arrival.time}</p>
                          <p className="text-gray-600">{flightInfo.arrival.date}</p>
                          <p className="text-lg font-medium text-gray-800 mt-1">{flightInfo.arrival.airport}</p>
                          <p className="text-sm text-gray-500">Terminal 7, Gate B15</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-200">
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{flightInfo.aircraft}</span>
                        </div>
                        <div className="flex justify-between">
                    ...
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-3">Check-in Information</h4>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Online Check-in:</span>
                          <span className="font-medium">24h before departure</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Airport Check-in:</span>
                          <span className="font-medium">2h before departure</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Boarding:</span>
                          <span className="font-medium">30 min before departure</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Gate Closes:</span>
                          <span className="font-medium">10 min before departure</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <TabsContent value="policies" className="mt-0">
                <div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold mb-4 text-gray-900">Baggage Policy</h3>
                  ...
                  <div>
                    <h3 className="text-lg font-semibold mb-4 text-gray-900">Cancellation & Changes</h3>
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                      ...
                      <p className="text-sm text-yellow-700">
                        Fare conditions vary by ticket type. Please review your specific fare rules before booking.
                      </p>
                    </div>
              ...
              <TabsContent value="amenities" className="mt-0">
                <div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold mb-4 text-gray-900">In-Flight Services</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {[
                        { icon: Wifi, title: "Free Wi-Fi", desc: "Complimentary high-speed internet" },
                        { icon: Monitor, title: "Entertainment", desc: "Personal screens with 1000+ options" },
                        { icon: Utensils, title: "Dining", desc: "Gourmet meals and premium beverages" },
                        { icon: Headphones, title: "Premium Audio", desc: "Noise-canceling headphones provided" },
                        { icon: Bed, title: "Comfort", desc: "Adjustable headrests and blankets" },
                        { icon: Zap, title: "Power Outlets", desc: "USB and power ports at every seat" }
                      ].map((amenity, idx) => (
                  ...
                  <div>
                    <h3 className="text-lg font-semibold mb-4 text-gray-900">Seat Configuration</h3>
              ...
              <TabsContent value="reviews" className="mt-0">
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                      { name: "Sarah Johnson", rating: 5, date: "Dec 2024", comment: "Excellent service and comfortable seats. The crew was very attentive throughout the flight." },
                      { name: "Michael Chen", rating: 4, date: "Nov 2024", comment: "Good value for money. Food could be better but overall a pleasant experience." },
                      { name: "Emma Davis", rating: 5, date: "Nov 2024", comment: "Loved the entertainment system and the legroom was generous for economy class." },
                      { name: "Robert Wilson", rating: 4, date: "Oct 2024", comment: "Flight was on time and the check-in process was smooth. Would fly again." }
                    ].map((review, idx) => (
