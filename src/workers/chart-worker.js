    ...
    return {
      ...
      monthlyTrend: {
        ...
        datasets: [{
          ...
          data: Object.keys(monthlyBookings).sort().map(month => monthlyBookings[month]),
          borderColor: 'rgb(75, 192, 192)',
          backgroundColor: 'rgba(75, 192, 192, 0.2)',
          tension: 0.1
        }]
...
function processFlightStats(data) {
  try {
    ...
    flights.forEach(flight => {
      ...
      // Route count
      const route = `${flight.departureAirport?.code || 'UNK'} - ${flight.arrivalAirport?.code || 'UNK'}`;
    ...
    return {
      registrationTrend: {
        ...
        datasets: [{
          ...
          data: Object.keys(monthlyRegistrations).sort().map(month => monthlyRegistrations[month]),
